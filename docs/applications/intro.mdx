...
A default project is created for you when [create your first application](./configuration), but you can create additional projects as needed.
...
Volumes are the preferred way to persist data in StateMesh. Volumes are mounted into containers and are backed by a storage provider. StateMesh supports two types of storage providers:
...
- **StateMesh**: volumes are provided by StateMesh using our network storage system. The data we store is encrypted and replicated across the network. This is typically used for sensitive data, because nodes never have access to this data.
